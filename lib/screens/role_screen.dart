  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: FutureBuilder(
            future:
                roles.where("restaurant_id", isEqualTo: RESTAURANT_ID).get(),
            builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
              if (snapshot.hasError) {
                // throw Exception("snapshot fetching from role screen has error");
                return Text("snapshot fetching from role screen has error");
              }
              if (!snapshot.hasData) {
              ...
              if (snapshot.connectionState == ConnectionState.done) {
                ...
                // this will always have one content since we considered restaurant_id as unique for all reference
                snapshotData!.docs.forEach((element) {
                  restaurantRoles = element;
                });
                ...
                print("restaurantRoles['roles'] is null");
...
// Future<List<Widget>> fetchRestaurantRoles() {
...
//     throw Exception(
//         "error occured while retrieving roles from database with restaurant_id");
//   });
//   return <Widget>[];
// }
